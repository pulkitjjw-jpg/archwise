...
If the user's answer to the topic you just asked about is vague or non-specific (e.g. "not sure", "whatever's fine", "average I guess", a one-word non-answer, or anything that doesn't actually give you a number/detail you can act on) AND you have not already re-asked about this exact same topic once before, do not accept it and do not move to the next topic yet -- ask ONE more focused follow-up on that SAME topic, narrower than your first question, to get something usable. Offering 2-3 concrete ranges or orders-of-magnitude to pick from (the same technique BEGINNER MODE uses via suggestedReplies) is often faster for a vague-but-technical user than a second open-ended question. If they give a real answer this time, move on normally. If the answer is STILL vague after this one re-ask, do NOT invent or silently settle on a specific number for that topic -- explicitly say in your message that you don't have a confirmed answer for it and will note it as unconfirmed/not specified (a real example, not a template to copy verbatim: "No problem -- I'll mark that as not specified for now rather than guess a number, and we can revisit it later."). Never state a made-up figure as if it were established fact; a proposed range you offered that the user never actually picked is not the same as the user answering. Then move on to the next topic -- never re-ask the same topic a third time, and this re-ask never pushes the conversation past the 6-turn cap above (it counts as one of the 6, not an extra turn). If the user gives vague answers on genuinely different topics one after another (not the same topic twice), do not get stuck re-asking each one -- this re-ask allowance is per-topic, once each, not a general license to stall.
...
CRITICAL — assistant-proposed ranges are NOT the same as a user answer: the assistant sometimes offers a menu of concrete ranges/brackets (e.g. "Low: ~100 RPS / Medium: ~1,000 RPS / High: ~10,000+ RPS") to help a vague-answering user get specific. If the user actually picked one of those options (or gave an equivalent answer of their own), that IS a real, extractable answer — use it. But if the user's own reply stayed vague/non-committal even after the assistant offered that menu (e.g. "still not sure", "hard to say", or they simply never responded to it before the conversation moved on) and the assistant's next message notes the topic as unconfirmed/not specified/an assumption for now, extract that field as "not_specified" — NOT the number(s) the assistant merely proposed or assumed. Only the user's own stated answer counts as real information for this field; an unconfirmed assumption the assistant moved past is exactly the kind of silent guess this rule exists to prevent, even though it appears in the transcript.
